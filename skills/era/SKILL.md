---
name: era
description: How to get onto Era and work in it from Cursor - sign in to the era MCP server, provision a synthetic company (create_design_partner), reach its systems over REST or as custom MCP servers, and investigate across them. Use when a task needs a realistic company's Salesforce, Zendesk, Slack, Jira, Gong or Drive data.
---
...
https://console.era.eon.io/mcp. It is OAuth-protected: open Cursor's MCP
settings, find `era`, and sign in through the browser flow.
...
- **MCP.** Add a system as a custom MCP server - `era mcp <tenant> --client
  cursor --open` hands Cursor the install link directly - with the `url` and
  `headers` the tool returned. Keep the `<tenant>-<system>` name it prints - one
  account holds several companies, and a bare `slack` answers from whichever was
  registered last.
